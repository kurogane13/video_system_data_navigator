        ...
        try:
            ...
            if mime_type and mime_type.startswith("video/"):
                ...
                debug_logger.log_action(
                    ...
                    f"curl -X GET 'http://192.168.0.144:9090/api/file?path={file_path}' -H 'Authorization: Bearer {self.get_auth_token()}'"
                )
    ...
    def handle_system_specs(self):
        ...
        try:
            ...
            debug_logger.log_action(
                ...
                f"curl -X POST 'http://192.168.0.144:9090/api/system-specs' -H 'Authorization: Bearer {self.get_auth_token()}'"
            )
    ...
    def handle_file_upload(self):
        ...
        try:
            ...
            debug_logger.log_action(
                ...
                f'curl -X POST http://192.168.0.144:9090/api/upload -H "Authorization: Bearer {self.get_auth_token()}" -F "video=@/path/to/your/video/{safe_filename}"'
            )
    ...
    def handle_general_upload(self):
        ...
        try:
            ...
            debug_logger.log_action(
                ...
                f'curl -X POST http://192.168.0.144:9090/api/general-upload -H "Authorization: Bearer {self.get_auth_token()}" -F "file=@/path/to/your/file/{safe_filename}"'
            )
    ...
    def handle_delete_file(self):
        ...
        try:
            ...
            debug_logger.log_action(
                ...
                f'curl -X POST http://192.168.0.144:9090/api/delete-file -H "Content-Type: application/json" -H "Authorization: Bearer {self.get_auth_token()}" -d "{{"path": "{file_path}"}}"'
            )
    ...
    def handle_files_zip_download(self):
        ...
        try:
            ...
            debug_logger.log_action(
                ...
                f"curl -X POST 'http://192.168.0.144:9090/api/download-files-zip' -H 'Authorization: Bearer TOKEN' -d '{json.dumps({'files': valid_paths})}'"
            )
...
def main():
    ...
    try:
        with ThreadedTCPServer(("0.0.0.0", PORT), AuthenticatedAPIHandler) as httpd:
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"[{timestamp}] 🔒 Secure API Server running at http://192.168.0.144:{PORT}")
            # Also log to debug log
            debug_logger.log_action("SERVER STARTED", f"Port: {PORT} |  Timestamp: {timestamp}")
            debug_logger.log_action("Authentication server initialized", f"Listening on: http://192.168.0.144:{PORT}")
